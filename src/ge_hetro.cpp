...
//	
 	//mask.resize(Nind, 1);
        ...
        else
        {
                ...
                cout<< "Read in environment "<<covname<<endl;
        }
...
///gxe computations
		...
		/* add hetero env noise
		 ...
		 hetro_all_zb=hetro_all_zb.colwise()*Enviro.col(env_index).array();
		 int hetro_index=Nbin+Nenv+env_index;
		  for (int z_index=0;z_index<Nz;z_index++){
			XXz.col(((hetro_index)*(Njack+1)*Nz)+(Njack*Nz)+z_index)=output_env.col(z_index);		 
			if(both_side_cov==true){
			   ...
			   hetro_all_zb=all_Uzb.array().colwise()*Enviro.col(env_index).array();
                	   hetro_all_zb=hetro_all_zb.colwise()*Enviro.col(env_index).array();
			   XXUz.col(((hetro_index)*(Njack+1)*Nz)+(Njack*Nz)+z_index)=hetro_all_zb.col(z_index);
			}
...
for (jack_index=0;jack_index<=Njack;jack_index++){
	...
	if(i>=(Nbin-(nongen_Nbin+Nenv)) ){
		...
		if(i==(Nbin-1)){
		  if(jack_index==Njack){
//		  cout<<"estimated trhht"<<b_trk(i,0)<<" true"<<Enviro.col(0).sum()<<endl;
		  }
		  //b_trk(i,0)=Enviro.col(0).sum();
...
/*outfile<<"Xl"<<endl<<X_l<<endl;
outfile<<"Yl"<<endl<<Y_r<<endl;*/
...
/*	  if(i>=(Nbin-nongen_Nbin-Nenv) && i<(Nbin-Nenv))
		point_est_adj_gxe(i,0)=point_est(i,0)*b_trk(i,0)/Nindv_mask;
	  else
		point_est_adj_gxe(i,0)=point_est(i,0);*/	
    } 
}
else{
	for(int i=0;i<(Nbin+1);i++)
...
/*     if(i>=(Nbin-nongen_Nbin-Nenv) && i<(Nbin-Nenv))
                jack_adj_gxe(i,jack_index)=jack(i,jack_index)*b_trk(i,0)/Nindv_mask;
     else
	 	jack_adj_gxe(i,jack_index)=jack(i,jack_index);
*/
}
}
